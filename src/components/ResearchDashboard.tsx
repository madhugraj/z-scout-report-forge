  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-[#1A1F2C] via-[#1E2330] to-[#1A1F2C] text-white overflow-hidden">
      ...
      <div className="flex flex-1 relative flex-col">
            ...
            <div className="bg-white overflow-auto h-full">
              {isGenerating && (
                <div className="bg-violet-100 p-6 text-violet-700 sticky top-0 z-10">
                  ...
                  <div className="mt-4 text-sm space-y-1 max-h-32 overflow-auto">
                    {generationSteps.map((step, index) => (
                      <div key={index} className="flex items-center gap-2">
                        {index === generationSteps.length - 1 ? (
                          <div className="h-3 w-3 bg-violet-500 rounded-full animate-pulse" />
                        ) : (
                          <div className="h-3 w-3 bg-violet-400 rounded-full opacity-50" />
                        )}
                        <p>{step}</p>
                      </div>
                    ))}
              ...
              <div className="p-8 max-w-4xl mx-auto">
                    ...
                    <div className="space-y-12">
                      {sections && sections.length > 0 ? (
                        sections.map((section, index) => (
                          <div 
                            key={index} 
                            className={`p-6 rounded-lg ${dropTargetIndex === index ? 'bg-violet-50 border-2 border-dashed border-violet-300' : 'bg-white'}`}
                        ...
                        <div className="text-center py-12">
                          <p className="text-gray-500">No sections generated yet. Try changing the research question.</p>
                          <div className="mt-4">
                            <Input
                            ...
                            />
                            <Button 
                              onClick={() => question && startGeneratingReport(question)} 
                              className="mt-2"
                            >
                ...
                {activeView === 'pdf-viewer' && (
                  <div className="py-4">
                    <h2 className="text-2xl font-bold text-gray-800 mb-6">Source PDFs</h2>
                        ...
                        <div key={index} className="bg-white border rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                          <div className="p-4">
                            <div className="flex gap-3">
                              <div className="bg-violet-100 text-violet-700 p-2 rounded">
                ...
                {activeView === 'images' && (
                  <div className="py-4">
                    <h2 className="text-2xl font-bold text-gray-800 mb-6">Research Images</h2>
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {topicSpecificData.images && topicSpecificData.images.map((image, index) => (
                        <div key={index} className="group relative rounded-lg overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow">
                          <img 
                            src={image.url} 
                            alt={image.title} 
                ...
                {activeView === 'citations' && (
                  <div className="py-4">
                    <h2 className="text-2xl font-bold text-gray-800 mb-6">Citations & References</h2>
                    <div className="space-y-4">
                      {mockReferences && mockReferences.map((reference, index) => (
                        <div key={index} className="bg-white p-4 rounded-lg border">
                          <h3 className="font-medium text-gray-900">{reference.title}</h3>
                          <p className="text-sm text-gray-600 mt-1">
                            {reference.authors.join(', ')} • {reference.year} • {reference.journal}
                          </p>
                          <p className="text-sm text-gray-500 mt-2">DOI: {reference.doi}</p>
                ...
                {activeView === 'tables' && (
                  <div className="py-4">
                    <h2 className="text-2xl font-bold text-gray-800 mb-6">Data Tables</h2>
                      ...
                      {topicSpecificData.tables && topicSpecificData.tables.map((table, index) => (
                        <div key={index} className="bg-white p-4 rounded-lg border">
                          <div className="flex gap-4 items-start">
                            ...
                            <div className="flex-1">
                              <h3 className="font-medium text-gray-900">{table.title}</h3>
                              <p className="text-sm text-gray-500 mt-1">{table.rows} rows • {table.columns} columns</p>
                              <div className="mt-3 flex gap-2">
                                <Button 
                                  variant="outline" 
                                  size="sm"
                            ...
                            <Button 
                              ...
                              size="icon"
                              onClick={() => {
                                // Add table insertion logic
                                if (sections && sections.length > 0) {
                                  const lastSection = sections[sections.length - 1];
                                  const tableHtml = `<div class="my-4 overflow-x-auto">
                                    <table class="w-full border-collapse">
                                      <caption class="text-sm text-left mb-2">${table.title}</caption>
                                      <thead>
                                        <tr class="bg-gray-100">
                                          ${Array(Math.min(5, table.columns)).fill(0).map((_, i) => `<th class="border px-4 py-2">Column ${i+1}</th>`).join('')}
                                        </tr>
                                      </thead>
                                      <tbody>
                                        ${Array(Math.min(5, table.rows)).fill(0).map((_, i) => `
                                          <tr>
                                            ${Array(Math.min(5, table.columns)).fill(0).map((_, j) => `<td class="border px-4 py-2">Row ${i+1}, Cell ${j+1}</td>`).join('')}
                                          </tr>
                                        `).join('')}
                                      </tbody>
                                    </table>
                                  ...
                                  toast.success(`Table added to ${lastSection.title}`);
                                }
                              }}
                            >
                ...
                {activeView === 'threads' && (
                  <div className="py-4">
                    ...
                    <div className="space-y-4">
                      {topicSpecificData.threads && topicSpecificData.threads.map((thread, index) => (
                        <div key={index} className="bg-white p-4 rounded-lg border hover:border-violet-300 transition-colors cursor-pointer">
                          <div className="flex gap-4 items-start">
                            <div className="bg-violet-100 text-violet-700 p-2 rounded">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-gray-500 mt-1">{thread.replies} replies • Last updated {thread.lastUpdated}</p>
                      ...
                      <Button className="w-full mt-4" onClick={() => setShowCollaborator(true)}>
                        <MessageSquare className="h-4 w-4 mr-2" />
        ...
        {activeSideView && (
          <div className="absolute top-0 right-0 z-10 w-80 h-full">
            {renderSidePanel()}
          </div>
        )}
      </div>
